...
class MultiScaleCoordinator:
    ...
    def __init__(self):
        ...
        self.cfa_neural = NeuralCFA(self.config)
        ...
        try:
            ...
            logger.warning(f"Neural CFA not available: {e}")
            self.cfa_neural = None
    ...
    def process_completed_route_with_cfa_learning(
        ...
        """
        Enhanced route completion processing with CFA learning

        This extends the existing process_completed_route() to also update CFA

        Args:
            route_outcome: Completed route data
            batch_formation: The batch decision that created this route
                            {'shipments': [...], 'vehicle': ..., 'pred_util': ..., 'pred_cost': ...}
        """
        # First do existing VFA update
        self.process_completed_route(route_outcome)

        # NEW: Add CFA learning if neural CFA is enabled
        if hasattr(self, "cfa_neural") and self.cfa_neural is not None:
            try:
                # Create CFA-friendly outcome format
                cfa_outcome = type(
                    "CFAOutcome",
                    (),
                    {
                        "actual_utilization": route_outcome.utilization,
                        "actual_cost": route_outcome.actual_cost,
                        "on_time_delivery_rate": (
                            1.0 if route_outcome.sla_compliance else 0.0
                        ),
                    },
                )()

                # Trigger CFA learning
                self.cfa_neural.learn_from_outcome(batch_formation, cfa_outcome)

                logger.info(f"CFA learned from route {route_outcome.route_id}")

            except Exception as e:
                logger.error(f"CFA learning failed: {e}", exc_info=True)
    ...
    def run_daily_strategic_update(
        ...
        analytics = DailyAnalytics(
            ...
            sla_compliance_rate=(
                sum(sla_compliant) / len(sla_compliant) if sla_compliant else 0.0
            ),
            cost_prediction_error=np.mean(cost_errors) if cost_errors else 0.0,
            duration_prediction_error=(
                np.mean(duration_errors) if duration_errors else 0.0
            ),
    ...
    def _batch_vfa_update(self, routes: List[RouteOutcome]):
        ...
        for route in routes:
            ...
            if not route.sla_compliance:
                actual_reward -= self.config.business_config.get(
        ...
        logger.info(
            f"Batch VFA update: {len(routes)} routes, avg TD: {avg_td_error:.2f}"
        )
    ...
    def _discover_route_patterns(self, routes: List[RouteOutcome]) -> List[Dict]:
        ...
        for r in routes:
            origin = (
                r.initial_state.get("origin_lat", 0),
                r.initial_state.get("origin_lon", 0),
            )
            dest = (
                r.initial_state.get("dest_lat", 0),
                r.initial_state.get("dest_lon", 0),
            )
            ...
            patterns.append(
                {
                    ...
                    "avg_duration_hours": np.mean(
                        [r.actual_duration_hours for r in cluster_routes]
                    ),
                }
            )
    ...
    def _analyze_utilization_by_day(
        ...
        return {
            day: np.mean(utils) if utils else 0.0 for day, utils in day_utils.items()
        }
    ...
    def _extract_customer_patterns(self, routes: List[RouteOutcome]) -> List[str]:
        ...
        if sla_rate < 0.8:
            insights.append(
                f"SLA compliance below target: {sla_rate:.1%} (target: 80%)"
            )
