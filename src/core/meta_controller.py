...
from .cfa import NeuralCFA, CFASolution
...
class MetaController:
    ...
    def __init__(self):
        ...
        self.cfa = NeuralCFA(self.config)
        self.vfa = NeuralVFA()
    ...
    def _assess_complexity(self, state: SystemState) -> ComplexityAssessment:
        ...
        if state.pending_shipments:
            avg_time_pressure = sum(
                s.time_pressure(state.timestamp) for s in state.pending_shipments
        ...
        # Complexity score
        complexity_score = (
            num_pending * 0.4 + num_available * 0.3 + avg_time_pressure * 10.0
        )
    ...
    def _assess_stakes(self, state: SystemState) -> StakesAssessment:
        ...
        if state.get_available_vehicles():
            avg_vehicle_cost = sum(
                v.fixed_cost_per_trip for v in state.get_available_vehicles()
            ) / len(state.get_available_vehicles())
            estimated_cost = (
                avg_vehicle_cost * len(state.pending_shipments) / 5
            )  # Rough estimate
        ...
        # Thresholds
        low_stakes_threshold = self.config.get(
            "dla.stakes_assessment.low_stakes_max_cost", 10000
        )
        high_stakes_threshold = self.config.get(
            "dla.stakes_assessment.medium_stakes_max_cost", 50000
        )
    ...
    def _use_pfa(self, state: SystemState) -> MetaDecision:
        ...
        if action.action_type in ["DISPATCH", "DISPATCH_IMMEDIATE"]:
            ...
            batch = {
                ...
                "shipments": (
                    action.shipments
                    if isinstance(action.shipments, list)
                    else [action.shipments]
                ),
    ...
    def _generate_candidate_actions(self, state: SystemState) -> list:
        ...
        if cfa_solution.status in ["OPTIMAL", "FEASIBLE"] and cfa_solution.batches:
            candidates.append({"type": "DISPATCH", "batches": cfa_solution.batches})

        # Candidate 2: Wait
        candidates.append({"type": "WAIT"})
