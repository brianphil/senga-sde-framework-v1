    ...
    def from_dict(cls, data: dict) -> "Shipment":
        ...
        return cls(
            ...
            declared_value=data.get(
                "declared_value", 0.0
            ),  # ← ADD THIS (with default for old records)
...
class VehicleState:
    ...
    def from_dict(cls, data: dict) -> "VehicleState":
        return cls(
            ...
            availability_time=(
                datetime.fromisoformat(data["availability_time"])
                if data.get("availability_time")
                else None
            ),
            home_location=(
                Location(**data["home_location"]) if data.get("home_location") else None
            ),
        )
...
class SystemState:
    ...
    def fleet_utilization(self) -> float:
        ...
        busy_vehicles = sum(
            1 for v in self.fleet_state if v.status not in [VehicleStatus.IDLE]
        )
    ...
    def from_dict(cls, data: dict) -> "SystemState":
        return cls(
            ...
            pending_shipments=[
                Shipment.from_dict(s) for s in data["pending_shipments"]
            ],
...
class StateManager:
    ...
    def save_route_formation_metadata(self, route_id: str, metadata: Dict):
        """Save batch formation metadata for CFA learning"""
        try:
            self.conn.execute(
                """
                INSERT OR REPLACE INTO route_metadata
                (route_id, formation_data, created_at)
                VALUES (?, ?, ?)
            """,
                (route_id, json.dumps(metadata), datetime.now().isoformat()),
            )
            self.conn.commit()
        except Exception as e:
            logger.error(f"Failed to save route metadata: {e}")
            ...
            try:
                ...
                logger.error(f"Failed to create metadata table: {e2}")

    def get_route_formation_metadata(self, route_id: str) -> Optional[Dict]:
        """Retrieve batch formation metadata for a route"""
        try:
            cursor = self.conn.execute(
                """
                SELECT formation_data FROM route_metadata
                WHERE route_id = ?
            """,
                (route_id,),
            )
            ...
            logger.error(f"Failed to get route metadata: {e}")
            return None
    ...
    def add_route(self, route: Route) -> bool:
        ...
        try:
            self.conn.execute(
                ...
                VALUES (?, ?, ?, ?)
            """,
                (route.id, route.vehicle_id, json.dumps(route.to_dict()), "active"),
            )
            self.conn.commit()
            self._invalidate_cache()
            return True
    ...
    def log_state_transition(
        self, decision_id: str, state_before: SystemState, action: dict
    ) -> int:
        """
        ...
        try:
            cursor = self.conn.execute(
                ...
                VALUES (?, ?, ?)
            """,
                (decision_id, json.dumps(state_before.to_dict()), json.dumps(action)),
            )
            self.conn.commit()
    ...
    def update_transition_outcome(
        ...
        try:
            self.conn.execute(
                ...
                WHERE id = ?
            """,
                (reward, json.dumps(state_after.to_dict()), transition_id),
            )
            self.conn.commit()
            return True
        except Exception as e:
            ...
            transitions.append(
                {
                    ...
                    "reward": row[2],
                    "state_after": (
                        SystemState.from_dict(json.loads(row[3])) if row[3] else None
                    ),
