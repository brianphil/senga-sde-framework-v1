...
class RouteSequenceOptimizer:
    ...
    def __init__(self, distance_calculator: DistanceTimeCalculator,
    ...
    def optimize_route_sequence(self,
                                origin: Location,
                                destinations: List[Location],
                                vehicle_capacity_m3: Optional[float] = None,
                                max_duration_hours: float = 8) -> RouteMetrics:
        """
        ...
        if ORTOOLS_AVAILABLE:
            try:
                return self._solve_with_ortools(origin, destinations, 
                                                vehicle_capacity_m3, max_duration_hours)
    ...
    def _simple_direct_route(self, origin: Location, destination: Location) -> RouteMetrics:
        """
        ...
        route_stops = [
            ...
            self._location_to_stop(destination, duration * 60, destination.shipment_ids)
        ]
    ...
    def _solve_with_ortools(self, origin: Location, destinations: List[Location],
                           vehicle_capacity: Optional[float],
                           max_duration: float) -> RouteMetrics:
        """
        ...
        while not routing.IsEnd(index):
            ...
            # Create stop
            route_stops.append(self._location_to_stop(
                location, cumulative_time, location.shipment_ids
            ))
            ...
            if not routing.IsEnd(next_index):
                ...
                segment_time = self.distance_calc.estimate_travel_time_hours(segment_distance)
        ...
        status = 'OPTIMAL' if solution.status == pywrapcp.RoutingModel.ROUTING_OPTIMAL else 'FEASIBLE'
    ...
    def _solve_with_greedy(self, origin: Location, destinations: List[Location]) -> RouteMetrics:
        """
        ...
        for i, location in enumerate(route):
            route_stops.append(self._location_to_stop(
                location, time_so_far, location.shipment_ids
            ))
            ...
            if i < len(route) - 1:
                ...
                segment_time = self.distance_calc.estimate_travel_time_hours(segment_dist)
    ...
    def _location_to_stop(self, location: Location, arrival_minutes: float,
                         shipment_ids: List[str] = None) -> RouteStop:
    ...
    def calculate_route_quality(self, route_stops: List[RouteStop]) -> Dict[str, float]:
        ...
        for i in range(len(route_stops) - 1):
            segment_dist = self.distance_calc.calculate_distance_km(
                route_stops[i].location_lat, route_stops[i].location_lon,
                route_stops[i+1].location_lat, route_stops[i+1].location_lon
            )
            total_distance += segment_dist
        
        total_duration = route_stops[-1].arrival_time_minutes / 60 if route_stops[-1].arrival_time_minutes else 0
...
def compare_routes(route1: RouteMetrics, route2: RouteMetrics) -> Dict[str, any]:
    ...
    - better_route (1 or 2)
    """
    distance_improvement = (route2.total_distance_km - route1.total_distance_km) / route1.total_distance_km * 100
    time_improvement = (route2.total_duration_hours - route1.total_duration_hours) / route1.total_duration_hours * 100
...
def validate_route_optimizer():
    ...
    print(f"   Duration: {result.total_duration_hours:.2f} hours")
    print(f"   Stops: {len(result.route_stops)}")
    print(f"   Route: {' → '.join([stop.location_address for stop in result.route_stops])}")
    ...
    print(f"   Optimized: {result.total_distance_km:.1f} km ({result.optimization_status})")
    print(f"   Greedy: {greedy_result.total_distance_km:.1f} km ({greedy_result.optimization_status})")
